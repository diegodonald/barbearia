...
export default function BarbeirosConfigPage() {
  const router = useRouter();
  const { user, loading: authLoading } = useAuth();
  const { barbeiros, loading: barbeirosLoading, error: barbeirosError } = useBarbeiros();
  ...
  const {
    ...
    saveOperatingHours,
    saveException,
  } = useOperatingHours(selectedBarberId);
  ...
  useEffect(() => {
    if (barbeirosError) {
      ...
      setFeedback(`Erro ao carregar barbeiros: ${barbeirosError}`);
  ...
  useEffect(() => {
    if (!configLoading && operatingHours) {
      setFormState(operatingHours);
    } else if (!configLoading && !operatingHours && selectedBarberId) {
      // Se não houver configuração para este barbeiro, criar uma padrão
      setFormState({
  ...
  }, [configLoading, operatingHours, selectedBarberId]);
  ...
  // Salvar as configurações do barbeiro
  const saveConfig = async () => {
    if (!formState || !selectedBarberId) return;
  ...
  const handleAddException = async () => {
    if (!newException.date || !selectedBarberId) {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">Configuração de Horários por Barbeiro</h1>

      {barbeirosError && (
        <div className="mb-4 p-3 bg-red-100 text-red-800 rounded">
          Erro ao carregar barbeiros: {barbeirosError}
        </div>
      )}
      ...
      <div className="mb-6">
        ...
        <select
          ...
          className="border rounded px-3 py-2 bg-gray-700 w-full md:w-1/2"
        >
        ...
        {barbeiros && barbeiros.length === 0 && !barbeirosLoading && (
          <p className="mt-2 text-yellow-500">
            Nenhum barbeiro encontrado. Verifique se existem usuários com role="barber" no banco de
            dados.
          </p>
        )}
      </div>
        ...
        <>
          {configLoading ? (
            ...
            <>
              {formState && (
                <div className="mb-8">
                  <h2 className="text-xl mb-4">Horários de Trabalho</h2>
                  {Object.keys(formState).map(day => renderDayForm(day as keyof OperatingHours))}
                  <button
                    onClick={saveConfig}
              ...
              <div className="mb-8">
                <h2 className="text-xl mb-4">Exceções (Férias, Folgas, Dias Especiais)</h2>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div className="col-span-2">
                      ...
                      <input
                        ...
                        className="border rounded px-2 py-1 bg-gray-700 w-full"
                        placeholder="Ex: Folga, Férias, etc."
                      />
                    </div>
                  </div>
                  <button
